...
# Each session type has an emoji, colour, and a study tip shown in the planner
SESSION_TYPES = {
...
# Default time blocks used when no specific start time is given
TIME_BLOCKS = [
    ("09:00", "Morning"),
    ("11:00", "Late Morning"),
    ("14:00", "Afternoon"),
...
@dataclass
class PlanItem:
    # Represents a single study session in the plan
    day: str           # YYYY-MM-DD
...
def _parse_days_until(text: str) -> Optional[int]:
    ...
    # Handle common phrases that don't have a number
    if "next week" in t:
        return 7
    if "a few days" in t:
        return 3
...
def _parse_hours_per_day(text: str) -> Optional[float]:
    # Extract how many hours per day the student wants to study
...
def _parse_start_time(text: str) -> Optional[str]:
    ...
    """
    t = text.lower()
    # Look for "study/start at X" or "from X" followed by a time
    m = re.search(
        r"(?:study|start|begin|from|at)\s+(?:at\s+)?(\d{1,2})(?::(\d{2}))?\s*(am|pm)",
        t, re.I
    )
    if not m:
        # Fall back to any time mention in the message
...
def _parse_modules(text: str) -> List[str]:
    ...
    # Words to ignore when extracting module names
    ...
    def strip_stops(words):
        # Remove stop words from the beginning of a word list
        while words and words[0].lower() in STOP:
            words = words[1:]
        return words

    # Pattern 1: Explicit "modules: X, Y, Z"
    ...
    # Pattern 2: "[words] SUBJECT exam" — grab words before the word "exam"
    ...
    # Pattern 3: "exam in/for/on Maths and Physics"
...
def _format_day_label(d: date) -> str:
    # Format a date as "Monday 21 Apr" for display in the planner
    return d.strftime("%A %-d %b")


def _add_minutes(time_str: str, minutes: int) -> str:
    # Add a number of minutes to a HH:MM string and return the result
...
def build_study_plan(message: str, default_start_time: str = "09:00") -> Tuple[Optional[Dict], Optional[str]]:
    ...
    # Extract all the key information from the student's message
    ...
    # Override default start time if the student specified one
    parsed_time = _parse_start_time(message)
    if parsed_time:
        default_start_time = parsed_time

    # Work out the exam deadline from either a date or a number of days
    if deadline_date:
    ...
    # Check what information is missing and ask the student for it
    missing = []
    if not modules:
        missing.append("your module(s) — e.g. *Modules: Maths, Physics*")
    if not hours:
        hours = 2.0  # default to 2 hours if not specified
    ...
    # Convert hours to minutes and cap between 45 and 180 minutes per session
    duration_min = max(45, min(180, int(hours * 60)))
    days_available = max(1, days_until)

    # Get the right session sequence based on how many days are available
    ...
    # Generate a plan item for each day from tomorrow until the exam
    for day_offset in range(days_available):
        ...
        if is_last_day:
            # Exam day — light review and confidence building only
        ...
        if is_pre_exam:
            # Day before exam — rest and very light revision only
            items.append(PlanItem(
                day=d.isoformat(),
    ...
    # Build the final plan dictionary to return to the frontend
    plan = {
