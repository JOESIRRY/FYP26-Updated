...
# Keywords that clearly mean the user wants a study plan built
# These trigger the own rule-based planner instead of the Claude API
PLAN_EXPLICIT = [
...
# Keywords that should NOT trigger the planner even if they sound similar
# For example "revision tips" should go to Claude, not the planner
PLAN_EXCLUSIONS = [
...
def _wants_plan(tag: str, message: str) -> bool:
    # Decide whether this message is requesting a study plan
    msg_l = message.lower().strip()

    # Check exclusions first — if the message asks for tips or advice, skip the planner
    for excl in PLAN_EXCLUSIONS:
        if excl in msg_l:
            return False

    # If the intent classifier said study_plan and there's time or schedule info, trigger planner
    if tag == "study_plan":
    ...
    # Also check the explicit keyword list regardless of what the intent classifier said
...
def generate_reply(profile: Dict[str, Any], tag: str, message: str, confidence: float) -> Dict[str, Any]:
    ...
    if _wants_plan(tag, message):
        ...
        if err:
            # Planner couldn't extract enough info — let Claude ask naturally
        ...
        # Plan was generated successfully — build the summary message
        s = plan.get("summary", {})
        inp = plan.get("inputs", {})
    ...
    # First retrieve relevant context from the knowledge base to help Claude
    ...
    # Send the message and KB context to Claude for a natural response
    reply = ollama_generate(SYSTEM_PROMPT, message, "\n".join(context_lines))
    if not reply:
        # If Claude fails for any reason use the fallback reply
