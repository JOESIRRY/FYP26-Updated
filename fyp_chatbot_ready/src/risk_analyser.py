...
RECOVERY_PATTERNS = [
    r"feel(ing)?\s+(much\s+)?(better|good|great|okay|fine|well)",
    r"way\s+better",
    r"im\s+(okay|fine|good|alright|great)",
    r"feeling\s+okay",
    r"im\s+okay",
    r"i\s+feel\s+better",
    r"things\s+are\s+(better|improving|looking up)",
    r"i.?m\s+(much\s+)?better",
    r"no longer\s+(feeling|want)",
    r"changed my mind",
    r"i.?m\s+safe",
    r"i\s+am\s+safe",
]

def _is_positive_recovery(messages: List[str]) -> bool:
    """Check if recent messages indicate recovery from crisis."""
    for msg in messages:
        text = msg.lower()
...
def _ml_sentiment_score(messages: List[str]) -> float:
    ...
    for i, msg in enumerate(valid_messages):
        # Recency weight: newest = 1.0, oldest = 0.1
        # Very strong recency so recent recovery messages quickly lower score
        weight = 0.1 + 0.9 * (i / max(n - 1, 1))
        ...
        if model:
            try:
                ...
                # Boost distress/anxious if clear distress words present
                # This fixes cases where ML underscores obvious distress
                DISTRESS_WORDS = ["overwhelmed", "depressed", "hopeless", "cant cope",
                                  "breaking down", "worthless", "exhausted", "burnout",
                                  "no motivation", "falling apart", "cant go on"]
                ANXIOUS_WORDS  = ["anxious", "panicking", "stressed", "nervous",
                                  "worried", "scared", "terrified", "panic attack"]

                msg_lower = msg.lower()
                if any(w in msg_lower for w in DISTRESS_WORDS):
                    distress_p = max(distress_p, 0.65)
                    neutral_p  = min(neutral_p, 0.15)
                elif any(w in msg_lower for w in ANXIOUS_WORDS):
                    anxious_p  = max(anxious_p, 0.55)
                    neutral_p  = min(neutral_p, 0.20)

                msg_contribution = (distress_p * 30) + (anxious_p * 18) - (neutral_p * 25)
...
def analyse_risk_persistent(db_path: Path, user_id: str, current_messages: List[str]) -> RiskResult:
    ...
    """
    # Always check CURRENT message for crisis first
    ...
    # Get historical messages for sentiment scoring
    ...
    try:
        ...
        # Only check crisis in recent 3 historical messages
        # If someone said crisis words earlier but is recovering,
        # dont keep showing Crisis — show their current state instead
        recent_historical = historical[-3:] if len(historical) >= 3 else historical
        if _is_crisis(recent_historical) and not _is_positive_recovery(current_messages):
            return RiskResult(score=90, level="crisis", label="Crisis", color="#ef4444", is_crisis=True)
