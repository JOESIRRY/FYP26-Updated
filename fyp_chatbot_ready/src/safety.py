...
# List of regex patterns that match crisis and self-harm language
# Covers common phrasings including spaced variants like "kill my self"
# and Arabic phrases for multilingual support
# This runs on every single message before anything else
_CRISIS_PATTERNS = [
...
@dataclass
class SafetyResult:
    # Simple result object returned by the safety check
    # is_crisis is True if any pattern matched
    is_crisis: bool
...
def check_safety(message: str) -> SafetyResult:
    ...
    # Convert to lowercase so matching works regardless of capitalisation
    text = message.lower().strip()
    for pat in _CRISIS_PATTERNS:
        if re.search(pat, text, re.IGNORECASE):
            # Pattern matched — return crisis result immediately
            return SafetyResult(True, "crisis_keywords")
    # No crisis language found — safe to continue processing
    return SafetyResult(False)


def crisis_response(user_location: str = "UK") -> str:
    """UK-first crisis response with proper resources."""
    # Returns a pre-written response with UK emergency contacts
    # This is always shown when crisis language is detected
    # The LLM never handles these messages
    return (
